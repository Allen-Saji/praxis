-- Reassert late-added integrity protections for databases that recorded the
-- earlier migration before those protections were reviewed. This migration is
-- additive and idempotent; 0006 remains the owner of auth audit nullability.
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE "spend_intents" ADD CONSTRAINT "intent_reasoning_keys_check"
...
BEGIN
  IF NEW.wallet_policy_version_id IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM "policy_versions" pv INNER JOIN "policy_scopes" ps ON ps.id = pv.scope_id
    WHERE pv.id = NEW.wallet_policy_version_id AND ps.organization_id = NEW.organization_id
      AND ps.scope_type = 'wallet' AND ps.wallet_id = NEW.wallet_id
  ) THEN
  ...
  IF NEW.assignment_policy_version_id IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM "policy_versions" pv INNER JOIN "policy_scopes" ps ON ps.id = pv.scope_id
    WHERE pv.id = NEW.assignment_policy_version_id AND ps.organization_id = NEW.organization_id
      AND ps.scope_type = 'assignment' AND ps.assignment_id = NEW.assignment_id
  ) THEN
