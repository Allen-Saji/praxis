...
export default async function Activity({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ before?: string; agent?: string; state?: string; period?: string; attention?: string }> }) {
  ...
  const state = states.find((item) => item === query.state);
  ...
  const result = await repository.decisionsForMember({ organizationId: context.organization.id, userId: context.session.user.id, agentId: query.agent, includeAgentOptions: true, state, since, attention: query.attention === "1", before: decodeCursor(query.before) });
  const agentId = result?.invalidAgent ? undefined : query.agent;
