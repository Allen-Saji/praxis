...
export default async function Wallets({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params; const session = await requireOwnerSession(); const data = await workspaceRepository().walletListForMember(slug, session.user.id); if (!data) notFound();
